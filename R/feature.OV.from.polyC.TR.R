...
feature.OV.from.polyC.TR <- function(feature.df, TR.EX, EX.only=F, feature.colname='gene', antisense=F,
  ...
  tr.ov <- tr.ov[tr.ov$seqnames.feature == tr.ov$seqnames.TR, ]
  tr.ov <- tr.ov  %>% dplyr::select(!('seqnames.feature'))
  ...
  ex.ov.sum <- tr.ov %>% group_by(TR_ID, EX_ID, strand.TR) %>% 
    summarise(seqnames, TR_ID, EX_ID, start.TR, end.TR, strand.TR, feature,  start.feature, end.feature, strand.feature, feature_n=seq_along(feature))
  ...
  ex.ov.sp  <- spread(ex.ov.sum[, c('seqnames', 'TR_ID', 'EX_ID', 'start.TR', 'end.TR', 'strand.TR','feature','feature_n')], feature_n, feature)
  
  
  ## Transcript-level features
  tr.ov.sum <- tr.ov %>% group_by(TR_ID, strand.TR) %>% 
    summarise(seqnames, TR_ID, feature,  start.feature, end.feature, strand.feature, feature_n=seq_along(feature))
  ...
  tr.ov.sp  <- spread(tr.ov.sum[, c('seqnames', 'TR_ID', 'strand.TR','feature', 'feature_n')], feature_n, feature)
