...
make.genome.plot.data <- function(feature.df, feature.col=5, feature.name='gene', visfrom=NA, visto=NA) {
  ...
  genome.plotdata$prime5[genome.plotdata$strand == '-'] <- genome.plotdata$end[genome.plotdata$strand   == '-']
  genome.plotdata$prime3[genome.plotdata$strand == '-'] <- genome.plotdata$start[genome.plotdata$strand == '-']
  genome.plotdata$prime5[genome.plotdata$strand == '+'] <- genome.plotdata$start[genome.plotdata$strand == '+']
  genome.plotdata$prime3[genome.plotdata$strand == '+'] <- genome.plotdata$end[genome.plotdata$strand   == '+']
